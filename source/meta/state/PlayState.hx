...
class PlayState extends MusicBeatState
{
	...
	private var songinfonew:FlxText;
	private var composerslol:String;
	...
	public static var enablescore:Bool = false;

	public static var extraAnim:String = '';
	...
	public static var characterColorOverrideLol:FlxColor;
...
//	public static var fuckyoulol:Array<FlxCamera> = [];
	...
	function goodNoteHit(coolNote:Note, character:Character, characterStrums:Strumline, ?canDisplayJudgement:Bool = true)
	{
		if (!coolNote.wasGoodHit) {
			...
			if (character.curCharacter == 'mom-car' && curSong.toLowerCase() == 'mil' && curStep >= 288 && health > 0.415)
				health -= 0.015;
	...
	function characterPlayAnimation(coolNote:Note, character:Character)
	{
		...
		var baseString = 'sing' + UIStaticArrow.getArrowFromNumber(coolNote.noteData).toUpperCase() + extraAnim;
	...
	override function stepHit()
	{
		...
		boyfriend.color = characterColorOverrideLol;
	...
	private function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			{
				case 0:
					if (!skipCountdown) FlxG.sound.play(Paths.sound('intro3-' + assetModifier), 0.6);
					Conductor.songPosition = -(Conductor.crochet * 4);
				case 1:
					...
					if (!skipCountdown) FlxG.sound.play(Paths.sound('intro2-' + assetModifier), 0.6);
				...
				case 2:
					...
					if (!skipCountdown) FlxG.sound.play(Paths.sound('intro1-' + assetModifier), 0.6);
				...
				case 3:
					...
					go.screenCenter();
					if (!skipCountdown) add(go);
					FlxTween.tween(go, {y: 300, alpha: 0}, Conductor.crochet / 1000, {
						ease: FlxEase.cubeInOut,
						onComplete: function(twn:FlxTween)
						{
							go.destroy();
					...
					if (!skipCountdown) FlxG.sound.play(Paths.sound('introGo-' + assetModifier), 0.6);
	...
	private function startSecondCountdown():Void
		{
			swagCounter = 0;
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
