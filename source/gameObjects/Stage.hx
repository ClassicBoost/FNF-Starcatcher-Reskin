...
class Stage extends FlxTypedGroup<FlxBasic>
{
	...
	var phillybg:FNFSprite;
	var city:FNFSprite;
	var streetBehind:FNFSprite;
	var street:FNFSprite;

	public static var spacelol:FNFSprite;
	...
	public function new(curStage)
	{
		...
		if (PlayState.determinedChartType == "FNF")
		{
			...
			{
				case 'spookeez' | 'south' | 'sugar-rush':
					curStage = 'spooky';
		...
		switch (curStage)
		{
			case 'spooky':
				...
				spacelol = new FNFSprite(-300, -300).loadGraphic(Paths.image('backgrounds/' + curStage + '/Space'));
				spacelol.antialiasing = true;
				spacelol.scrollFactor.set(0.01, 0.01);
				spacelol.setGraphicSize(Std.int(spacelol.width * 1));
				spacelol.visible = false;
				add(spacelol);
			...
			case 'philly':
				...
				phillybg = new FNFSprite(-100).loadGraphic(Paths.image('backgrounds/' + curStage + '/sky'));
				phillybg.scrollFactor.set(0.01, 0.01);
				add(phillybg);

				city = new FNFSprite(-10).loadGraphic(Paths.image('backgrounds/' + curStage + '/city'));
				city.scrollFactor.set(0.05, 0.05);
				...
				for (i in 0...5)
				{
					...
					light.scrollFactor.set(0.05, 0.05);
					light.visible = false;
				...
				streetBehind = new FNFSprite(-40, 50).loadGraphic(Paths.image('backgrounds/' + curStage + '/behindTrain'));
				add(streetBehind);
				...
				street = new FNFSprite(-40, streetBehind.y).loadGraphic(Paths.image('backgrounds/' + curStage + '/street'));
				add(street);
			case 'highway':
				curStage = 'highway';
				...
				skyBG.scrollFactor.set(0.001, 0.001);
				add(skyBG);
				...
				add(grpLimoDancers); // wouldn't matter, there isn't any
	...
	public static function hidespacelol() {
		spacelol.visible = false;
	}
	public static function showspacelol() {
	...
	public static function phillyMoment() {
		if (PlayState.funnyPhillyGlow) {
			if (curLight == 0) PlayState.characterColorOverrideLol = 0xFF31A2FD;
			else if (curLight == 1) PlayState.characterColorOverrideLol = 0xFF31FD8C;
			else if (curLight == 2) PlayState.characterColorOverrideLol = 0xFFFB33F5;
			else if (curLight == 3) PlayState.characterColorOverrideLol = 0xFFFD4531;
			else if (curLight == 4) PlayState.characterColorOverrideLol = 0xFFFD4531;
		}
	}
	...
	public function stageUpdate(curBeat:Int, boyfriend:Boyfriend, gf:Character, dadOpponent:Character)
	{
		...
		//		bgGirls.dance();
			...
			case 'philly':
				...
				if (curBeat % 8 == 4 && FlxG.random.bool(30) && !trainMoving && trainCooldown > 8 && !PlayState.funnyPhillyGlow)
				{
	...
	public function stageUpdateConstant(elapsed:Float, boyfriend:Boyfriend, gf:Character, dadOpponent:Character)
	{
		...
		{
			case 'philly':
				if (trainMoving && !PlayState.funnyPhillyGlow)
				{
	...
	function updateTrainPos(gf:Character):Void
	{
		...
		if (startedMoving && !PlayState.funnyPhillyGlow)
		{
